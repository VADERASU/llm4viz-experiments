...
for i in range(len(qaShuffleList)):
    ...
    prompt = 'You are a helpful assistant for analyzing data visualizations. Please answer with the letter corresponding to the best option, or make a random guess if unsure. For example, if option (a) is correct, only reply with (a).'
    failBool = True
    while failBool:
        try:
            start = time.time()
    ...
    with open('../Results/Gemini/reVLAT_Viz+Choices/'+str(i)+'_'+chartKey+'_'+qaKey+'.json', 'w') as outfile:
