...
for i in range(4200, len(qaShuffleList)):
    ...
    response = client.chat.completions.create(model='gpt-4-turbo-preview',
                                            messages=[{'role': 'system',
                                                        'content': [{'type': 'text',
                                                                    'text': 'You are a helpful assistant for answering questions. Please answer with the letter corresponding to the best option, or make a random guess if unsure. For instance, if option (a) is correct, please reply with (a).'}]},
                                                        {'role': 'user',
                                                        'content': [{'type': 'text',
                                                                    'text': qaInput}]}],
                                            max_tokens=300)
    ...
    with open('../Results/ChatGPT/reVLAT_NoViz+Choices/'+str(i)+'_'+chartKey+'_'+qaKey+'.json', 'w') as outfile:
